...
async function loadData() {
  ...
  try {
    // Kita tetap mengambil data relawan untuk mengisi dropdown nama peminjam
    ...
    document.getElementById('loading').innerHTML = `
      <div class="text-center py-10 bg-white rounded-2xl shadow-sm border border-slate-100">
        ...
        <p class="text-slate-800 font-bold mb-1">Gagal Terhubung</p>
        <p class="text-slate-500 text-xs mb-4 px-4">Pastikan koneksi internet stabil atau URL API sudah benar.</p>
        <button onclick="loadData()" class="bg-slate-800 hover:bg-slate-700 text-white px-5 py-2.5 rounded-xl text-xs font-bold transition-all">Coba Muat Ulang</button>
      </div>`;
  }
}
...
function setViewMode(mode) {
  ...
  if(mode === 'grid') {
    ...
    listBtn.className = "w-10 h-10 flex items-center justify-center rounded-lg text-sm font-bold text-slate-400 hover:text-slate-600 transition-all";
  } else {
    ...
    gridBtn.className = "w-10 h-10 flex items-center justify-center rounded-lg text-sm font-bold text-slate-400 hover:text-slate-600 transition-all";
  }
  renderData();
}
...
function setCategory(category, el) {
  ...
  document.querySelectorAll('.cat-chip').forEach(chip => {
    chip.className = "cat-chip px-4 py-1.5 rounded-full text-xs font-bold whitespace-nowrap bg-white text-slate-500 border border-slate-200 hover:bg-slate-50 transition-all";
  });
...
function renderData() {
  ...
  if(currentViewMode === 'grid') {
    content.className = "grid grid-cols-1 sm:grid-cols-2 gap-4"; // 1 kolom di HP kecil, 2 di tablet
  } else {
  ...
  inventoryData.forEach(item => {
    ...
    if(dipinjamOleh) {
        infoPeminjam = `
        <div class="mt-2.5 bg-orange-50/80 border border-orange-100 rounded-lg p-2 flex items-start gap-2">
           <i class="fa-solid fa-hand-holding-hand text-orange-400 text-xs mt-0.5"></i>
           <div class="text-[11px] text-orange-800 font-medium leading-snug">
    ...
    if(currentViewMode === 'grid') {
      content.innerHTML += `
        <div class="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm flex flex-col justify-between relative group hover:shadow-md transition-all">
          ...
          <div>
            <div class="flex justify-between items-center text-[10px] mb-2">
              <span class="text-slate-400 font-bold flex items-center gap-1.5 bg-slate-50 px-2 py-1 rounded-md"><i class="fa-solid fa-location-dot"></i> ${item.Lokasi_Simpan || 'Gudang'}</span>
              <span class="font-black text-slate-300 uppercase tracking-wider">${item.Kategori}</span>
            </div>
            
            <h4 class="font-extrabold text-slate-800 text-base mb-1 leading-tight pr-2">${item.Nama_Barang}</h4>
            ...
            <div class="flex items-baseline gap-1.5 mb-2">
                ...
                <span class="text-[10px] text-slate-300 ml-1">(dari ${total})</span>
            </div>
            
            ${infoPeminjam}
          </div>

          <div class="flex gap-2 pt-4 mt-3 border-t border-slate-50">
            <button onclick="openPinjamModal('${item.ID_Barang}', '${item.Nama_Barang}', 1, '${item.Satuan}')" class="flex-1 bg-rose-50 hover:bg-rose-100 text-rose-600 py-2 rounded-xl font-bold text-xs transition-colors"><i class="fa-solid fa-minus mr-1"></i> Pakai</button>
            <button onclick="openPinjamModal('${item.ID_Barang}', '${item.Nama_Barang}', -1, '${item.Satuan}')" class="flex-1 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 py-2 rounded-xl font-bold text-xs transition-colors"><i class="fa-solid fa-plus mr-1"></i> Kembali</button>
          </div>
        </div>`;
    } else {
      // List View
      content.innerHTML += `
        <div class="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm flex items-center gap-4 hover:shadow-md transition-all relative overflow-hidden">
          ...
          <div class="flex-1 min-w-0 pl-1">
            ...
            <h4 class="font-extrabold text-slate-800 text-sm truncate mb-0.5">${item.Nama_Barang}</h4>
            
            <div class="flex items-center gap-3">
              <span class="text-sm font-black ${sisa > 0 ? 'text-emerald-500' : 'text-rose-500'}">${sisa} <span class="text-xs font-bold text-slate-400">${item.Satuan || ''}</span></span>
            </div>
            
            ${infoPeminjam}
          </div>
          
          <div class="flex flex-col gap-2 shrink-0 border-l border-slate-100 pl-4">
            <button onclick="openPinjamModal('${item.ID_Barang}', '${item.Nama_Barang}', 1, '${item.Satuan}')" class="w-24 bg-rose-50 hover:bg-rose-100 text-rose-600 py-2 rounded-xl font-bold text-[11px] transition-colors"><i class="fa-solid fa-minus mr-1"></i> Pakai</button>
            <button onclick="openPinjamModal('${item.ID_Barang}', '${item.Nama_Barang}', -1, '${item.Satuan}')" class="w-24 bg-emerald-50 hover:bg-emerald-100 text-emerald-600 py-2 rounded-xl font-bold text-[11px] transition-colors"><i class="fa-solid fa-plus mr-1"></i> Kembali</button>
          </div>
        </div>`;
    }
  });
...
function openPinjamModal(id, namaBarang, aksi, satuan) {
    ...
    const labelNama = document.getElementById('labelNamaPeminjam');
    ...
    if (aksi === 1) {
        ...
        desc.innerHTML = `Barang: <strong class="text-slate-800">${namaBarang}</strong>. Berapa jumlah yang ingin dipakai dan siapa yang bawa?`;
        btn.className = "w-full text-white font-bold py-3.5 rounded-xl text-sm shadow-lg transition-all active:scale-95 bg-rose-500 hover:bg-rose-600 shadow-rose-200";
        btn.innerText = "Konfirmasi Pakai";
        labelNama.innerText = "Siapa yang Pakai?";
    } else {
        ...
        desc.innerHTML = `Barang: <strong class="text-slate-800">${namaBarang}</strong>. Berapa jumlah yang dikembalikan dan siapa yang mengembalikan? (Pilih nama yang meminjam agar dihapus dari catatan).`;
        btn.className = "w-full text-white font-bold py-3.5 rounded-xl text-sm shadow-lg transition-all active:scale-95 bg-emerald-500 hover:bg-emerald-600 shadow-emerald-200";
        btn.innerText = "Konfirmasi Kembali";
        labelNama.innerText = "Siapa yang Mengembalikan?";
    }
...
async function submitPinjam(event) {
    ...
    const baseAksi = Number(document.getElementById('pinjamAksi').value); // 1 atau -1
    const qtyInput = Number(document.getElementById('pinjamQty').value); // Jumlah angka yg diinput
    ...
    // Kalkulasi final change amount (misal: ambil 3 = 1 * 3 = 3. Kembali 2 = -1 * 2 = -2)
    ...
    if (item) {
       ...
       if (dp > Number(item.Total_Stok)) {
           alert(`Gagal! Stok tersedia tidak cukup. Anda mencoba memakai ${qtyInput}, tapi sisa stok hanya ${item.Sisa_Stok}.`);
           closePinjamModal();
           btn.disabled = false;
           return;
       }
       ...
       if (baseAksi === 1) { // Pinjam
           item.Dipinjam_Oleh = listStr ? `${listStr}, ${namaPeminjam} (${qtyInput})` : `${namaPeminjam} (${qtyInput})`;
       } else { // Kembali
           // Menghapus nama secara spesifik dari UI sangat rumit karena string.
           // Kita timpa saja dengan tulisan loading agar tau sedang sinkronisasi.
    ...
    try {
      ...
      loadData(); // Tarik data asli terbaru dari server
    } catch(e) { 
      showToast("Gagal menyimpan. Cek koneksi Anda.");
    } finally {
      btn.disabled = false;
    }
}
